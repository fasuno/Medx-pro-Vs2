...
Public Class FrmPrinlabrqst
    ...
    End Sub
    Private Sub FrmPrinlabrqst_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        showCompDetails()
    ...
    Public Sub Showlabtest()
        ...
        cmd = New SqlCommand("Select Test_Name, Comment from LabRequest WHERE Request_Num= '" & LBLID.Text & "'", con)

        Dim adpter As New SqlDataAdapter(cmd)

        adpter.Fill(btbls)

        Dgvlabprint.DataSource = btbls

        Dgvlabprint.Height += 19
        If Y = 19 Then

            Y += 19
            Panel1.Height += Y
            Me.Height += Y
            Dgvlabprint.Height += Y
        End If
    End Sub
    ...
    Private Sub Dgvlabprint_RowsAdded(sender As Object, e As DataGridViewRowsAddedEventArgs)
